...
const handleRequest = async (req) => {
  ...
  try {
    ...
    if (!isManualHomeShipment(shipment)) {
      throw new Error("Only HOME/MANUAL shipments can be manually updated");
    }
    if (text(shipment.shipping_status, 40).toLowerCase() !== "created") {
      throw new Error("Only created shipments can be manually shipped");
    }

    const trackingNo = text(body.trackingNo, 80);
    if (!trackingNo) {
      throw new Error("trackingNo is required");
    }
